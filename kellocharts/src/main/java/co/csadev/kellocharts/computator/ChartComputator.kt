...
open class ChartComputator {
    ...
    fun setViewportTopLeft(left: Float, top: Float) {
        ...
        mLeft = Math.max(maximumViewport.left, Math.min(mLeft, maximumViewport.right - curWidth))
        mTop = Math.max(maximumViewport.bottom + curHeight, Math.min(mTop, maximumViewport.top))
        constrainViewport(mLeft, mTop, mLeft + curWidth, mTop - curHeight)
    }
